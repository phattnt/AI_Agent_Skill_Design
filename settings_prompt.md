Generate a complete desktop-first web application view for the "Settings Page" of the "Packing Video Proof App". This page allows users to configure application hardware and subscription defaults.
...
**IMPORTANT:** Use the exact HTML structure and Tailwind classes provided below for the SideNavBar and TopNavBar to maintain strict consistency with the Dashboard layout. Note that the active state for the sidebar link should be set to "Cài đặt".
...
For the actual page content inside the layout structure above, construct a split bento layout (e.g., 2 evenly sized columns or varied rows) containing:

1. **Hardware Configuration Card:** A bento card with forms for setting the Webcam Source dropdown, Video Resolution quality toggles, and Audio input selectors.
2. **Billing & Storage Card:** A sleek card providing Billing details, featuring a visually prominent Storage Plan progress bar and an 'Upgrade CTA' primary gradient button.
3. **E-commerce Integrations Card:** A list of switches to easily toggle connections on/off for Shopee, TikTok Shop, and Lazada.
