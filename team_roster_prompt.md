Generate a complete desktop-first web application view for the "Team Roster Page" of the "Packing Video Proof App". This page allows E-commerce managers to view warehouse staff performance and team data.
...
**IMPORTANT:** Use the exact HTML structure and Tailwind classes provided below for the SideNavBar and TopNavBar to maintain strict consistency with the Dashboard layout. Note that the active state for the sidebar link should be set to "Đội ngũ".
...
### 3. Team Roster Content Area
For the actual page content inside the layout structure above, construct a crisp bento layout containing:

1. **Header Area:** A title "Quản lý đội ngũ" (Team Management) along with a primary CTA button "+ Thêm nhân viên" (Add Employee).
2. **Team Member Grid:** A multi-column grid of employee profile bento cards.
   - Each card contains a perfect-circle avatar, employee name, and role/station.
   - Metrics at the bottom of the card displaying "Total Packed" and "Avg Pace/Order" in an elegant data display.
   - Include a "View Stats" pill button on hover.
