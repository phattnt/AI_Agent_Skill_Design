Generate a complete desktop-first web application view for the "Recording Studio Page" of the "Packing Video Proof App". This page allows E-commerce sellers to record packing videos via Webcam (triggered by scanning QR codes) to prevent false refund claims.
...
**IMPORTANT:** Use the exact HTML structure and Tailwind classes provided below for the SideNavBar and TopNavBar to maintain strict consistency with the Dashboard layout. Note that the active state for the sidebar link should be set to "Phòng quay video".
...
```html
    <aside
        ...
        <nav class="flex-1 space-y-2">
            <a class="flex items-center gap-3 px-4 py-2 text-gray-600 hover:bg-white/40 rounded-full text-sm font-medium tracking-tight transition-colors duration-200" href="#">
                <span class="material-symbols-outlined">dashboard</span>
                <span>Tổng quan</span>
            </a>
            <a class="flex items-center gap-3 px-4 py-2 text-blue-800 bg-white/50 shadow-sm rounded-full text-sm font-medium tracking-tight transition-all" href="#">
            ...
            <div class="flex items-center gap-6">
                ...
                <div class="flex items-center gap-3">
                    <div class="text-right">
                        ...
                        <p class="text-[10px] text-gray-600 mt-1">Ops Manager</p>
                    </div>
...
### 3. Recording Studio Content Area
For the actual page content inside the layout structure above, construct a crisp bento grid layout (2/3 + 1/3 columns) containing:

1. **Left Container (Camera Screen):**
   - A massive central card displaying a webcam placeholder frame.
   - Include a target overlay in the center for QR code scanning.
   - A dynamic status badge at the top right of the video feed ("Waiting for QR..." in gray/yellow vs a red pulsing "Recording Order #12345" badge).

2. **Right Container (Sidebar Panels):**
   - **Current Order Details Card:** A bento card showing the SKU list / product images to be packed for the currently scanned order. Include a primary button "Complete Order".
   - **Recent History Card:** A bento list showing recently packed orders with green success checkmarks indicating successful recording upload.

---
💡 Ensure strict adherence to the Luminous Bento design system and bento grid layout without any drop-shadows. Use only 1px borders and distinct padding arrays.
