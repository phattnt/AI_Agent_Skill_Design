Generate a complete desktop-first web application view for the "Product Catalog Page" of the "Packing Video Proof App". This page allows E-commerce sellers and packers to view the catalog of items to identify them accurately during packing.
...
**IMPORTANT:** Use the exact HTML structure and Tailwind classes provided below for the SideNavBar and TopNavBar to maintain strict consistency with the Dashboard layout. Note that the active state for the sidebar link should be set to "Danh mục".
...
For the actual page content inside the layout structure above, construct a crisp bento grid layout containing:

1. **Top Actions Bar:** A bento card with filter dropdowns (Categories, Stock Status) and a button to "Sync with Shopee/TikTok/Lazada".
2. **Product Grid:** A masonry or standard bento grid of product reference cards.
   - Each card displays a clear, centered product image and white background.
   - Below the image, show the bold SKU, Product Title, and inventory stock number.
   - A tiny barcode icon linking to "Print QR" should be available on each card.
